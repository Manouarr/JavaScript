 ...
 ************************************/
/**
 * Fonction qui retourne un nombre entier aléatoire compris entre un manimum et maximum
 * @param  {number} min
 * @param  {number} max
 */
 ...
 * Vérifie qu'il n'y ait pas de doublons et que les chiffres soient dans le bon intervalle
 */
 ...
 */
function generateComputeTirage() {
  // on tire 5 chiffres aléatoires uniques compris entre 1 et 50
  let nombre;
  while (tirage.size < 5) {
    nombre = getRandomInteger(1, 50);
    if (tirage.has(nombre) == false) {
      // si le nombre n'est pas déjà dans le set, on l'ajoute
      tirage.add(nombre);
    }
  }
...
function genPrice() {
  ...
  if (gain > 0) {
    ...
    document.querySelector("#result h3").textContent =
      "Vous n'avez rien gagné, mais vous pouvez retenter votre chance !";
  }
}
